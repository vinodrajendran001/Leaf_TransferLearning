...
def save_bottlebeck_features():
    ...
    bottleneck_features_train = model.predict_generator(train_datagen.flow(train_data, batch_size=32), nb_train_samples)
    ...
    bottleneck_features_validation = model.predict_generator(test_datagen.flow(test_data, batch_size=32),
...
def train_top_model():
    ...
    checkpointer = ModelCheckpoint(filepath="weights_leaf2.h5", monitor='loss',
                                   verbose=1, save_best_only=True)
    model.fit(train_data, train_labels,
              nb_epoch=nb_epoch, batch_size=32,
              callbacks=[TensorBoard(log_dir='leaf2'), checkpointer])
    ...
    fp = open('submission_nn_kernel_leaf2.csv', 'w')
